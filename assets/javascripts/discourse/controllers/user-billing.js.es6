...
export default Ember.Controller.extend({
  actions: {
    cancelSubscription: function(subscription){
      var self = this;
      return bootbox.confirm(I18n.t("memberships.cancel_confirmation"), I18n.t("no_value"), I18n.t("yes_value"), function(result) {
        if (result) {
    ...
    viewReceipt: function(receipt){
      window.location.href = "/memberships/transactions/" + this.currentUser.id + "/" + receipt.transaction_id;
    },

    updateBilling: function(subscription){
      if (Discourse.SiteSettings.memberships_gateway === "PayPal") {
        ...
        window.location.href = "/memberships/l/" + subscription.product.id;
      }
    }
  }
})
