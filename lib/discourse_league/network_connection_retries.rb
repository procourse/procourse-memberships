  ...
  module NetworkConnectionRetries
    ...
    def retry_exceptions(options={})
      ...
      retry_network_exceptions(options) do
        begin
          ...
          raise DiscourseLeague::RetriableConnectionError.new("The remote server refused the connection", e)
        rescue OpenSSL::X509::CertificateError => e
          NetworkConnectionRetries.log(options[:logger], :error, e.message, options[:tag])
          raise DiscourseLeague::ClientCertificateError, "The remote server did not accept the provided SSL certificate"
        rescue Zlib::BufError => e
          raise DiscourseLeague::InvalidResponseError, "The remote server replied with an invalid response"
        rescue *connection_errors.keys => e
          raise DiscourseLeague::ConnectionError.new(derived_error_message(connection_errors, e.class), e)
        end
      end
    end
    ...
    def retry_network_exceptions(options = {})
      ...
      begin
        ...
        result
      rescue DiscourseLeague::RetriableConnectionError => e
        retries -= 1

        log_with_retry_details(options[:logger], initial_retries-retries, Time.now.to_f - request_start, e.message, options[:tag])
        retry unless retries.zero?
        raise DiscourseLeague::ConnectionError.new(e.message, e)
      rescue DiscourseLeague::ConnectionError, DiscourseLeague::InvalidResponseError => e
        retries -= 1
        log_with_retry_details(options[:logger], initial_retries-retries, Time.now.to_f - request_start, e.message, options[:tag])
